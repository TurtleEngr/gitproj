...
=head1 Runnig git proj commands

=over 4

=item * Set up a project directory with some dummy files
...
=item * First we need to create a directory for the remote. We could mount
...
=item * Now we can run `git proj remote`
...
=item * Now let's try `git proj status` again
...
=item * Let's put a file in raw/. It doesn't have to be a binary file and it
...
=item * To "save" the raw/ files to remote-raw we run `git proj push`
...
=item * New let's change a file in raw/ and remove a file, then push.
...
=item * What about pull? If raw files in the remote are changed, added, or
...
=item * Now for the pull. It also has a '-d' option.
...
=item * That was a simulation of a remote-raw change. How about a
...
=item * We'll simulate this some by cloning into another place under the tmp
...
=item * Now you can do a `git proj status`
